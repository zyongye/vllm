...
class RoutingData:
    ...
    def n_blocks(self, n_rows, block_m):
        if n_rows <= self.n_expts_tot:
            ...
            return triton.cdiv(max(n_rows - self.n_expts_tot + 1, 0),
                               block_m) + self.n_expts_tot - 1
...
def routing(logits, n_expts_act, expt_indx=None):
    ...
    expt_scal = torch.empty((n_tokens, n_expts_act),
                            dtype=logits.dtype,
                            device=dev)
    expt_indx = torch.empty((n_tokens, n_expts_act),
                            dtype=torch.int16,
                            device=dev)
    bitmatrix = torch.empty((n_tokens, n_expts_words),
                            dtype=torch.uint32,
                            device=dev)
    tok_starts = torch.empty(n_expts_tot + 1, dtype=torch.int32, device=dev)
    partial_hist = torch.empty((cdiv(n_tokens, HIST1_BLOCK_M), n_expts_tot),
                               dtype=torch.int32,
                               device=dev)
    partial_offs = torch.empty((cdiv(n_tokens, HIST1_BLOCK_M), n_expts_tot),
                               dtype=torch.int32,
                               device=dev)
    # output tensors
    ...
    routing_details.bitmatrix._compute_bitmatrix[(cdiv(
        n_tokens, ROUTING_BLOCK_M), )](
            logits,
            logits.stride(0),  # inputs
            expt_scal,
            expt_indx,
            expt_scal.stride(0),  # output [topk]
            bitmatrix,
            bitmatrix.stride(0),  # output [bitmatrix]
            n_tokens,
            n_expts_tot,  # shapes
    ...
    routing_details.histogram._memset_hist[(cdiv(
        hist.shape[0], MEMSET_BLOCK), )](
            hist,
            hist.shape[0],
            tok_starts,  # outputs
            BLOCK=MEMSET_BLOCK  # tunable parameter
        )
    routing_details.histogram._compute_hist[(cdiv(
        n_tokens, HIST1_BLOCK_M), cdiv(n_expts_tot, HIST1_BLOCK_N))](
            bitmatrix,
            bitmatrix.shape[0],
            bitmatrix.stride(0),  # input
            hist,  # output [histogram]
            tok_starts,
            partial_hist,
            partial_hist.stride(0),
            partial_hist.shape[1],  # output [cumsums]
    ...
    return RoutingData(gate_scal, hist, n_expts_tot,
                       n_expts_act), gather_indx, scatter_indx
...
def routing_torch(logits, n_expts_act, expt_indx=None):
    ...
    hist = torch.histc(expt_indx, bins=n_expts_tot,
                       max=n_expts_tot - 1)  # histogram of tokens over experts
    # pack the matmul data structure
...
def simulate_expert_sharded_routing(n_global_rows,
                                    routing_data,
                                    n_expt_shards,
                                    row_align=1,
                                    device="cuda",
    ...
    if cache is None or n_global_rows not in cache:
        ...
        weights = torch.ones(n_global_rows,
                             routing_data.n_expts_tot,
                             device=device)
        expt_indx = torch.multinomial(weights,
                                      num_samples=routing_data.n_expts_act,
                                      replacement=False)

        # Sort each token's selections by expert.
        expt_indx, _ = expt_indx.sort(dim=1)

        hist = torch.histc(expt_indx,
                           bins=routing_data.n_expts_tot,
                           max=routing_data.n_expts_tot - 1)[:n_expts_local]
        ...
        n_local_rows = (
            (n_local_rows + row_align - 1) // row_align) * row_align

        is_active = torch.argsort((num_local_expts == 0).to(torch.int8),
                                  stable=True)
        ...
        topk_indx = torch.argsort(expt_indx.view(-1),
    ...
    tokens_per_expt = ((n_global_rows // n_expt_shards) *
                       routing_data.n_expts_act) // n_expts_local
