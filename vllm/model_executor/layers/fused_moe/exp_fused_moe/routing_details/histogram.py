...
def _compute_hist(
    ...
    r = tl.load(R + offs_m[None, :] * stride_rm + offs_b[:, None],
                mask=offs_m[None, :] < shape_rm)
    ...
    tl.debug_barrier()
    if tl.atomic_add(TokensStart,
                     1) != tl.num_programs(0) * tl.num_programs(1) - 1:
        return
    tl.debug_barrier()
