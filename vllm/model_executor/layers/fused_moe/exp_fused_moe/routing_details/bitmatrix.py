...
@triton.jit
def load_logits_topk(X, stride_xm, n_expts_tot, offs_m, mask_m,
                     N_EXPTS_PAD: tl.constexpr, N_EXPTS_ACT: tl.constexpr,
    ...
    for _i in range(loop_iterations):
        ...
        x = (x.to(tl.uint16, bitcast=True).to(tl.int32) <<
             16) | offs_x_n[None, :]
        x = x.to(tl.float32, bitcast=True)
...
def _compute_bitmatrix(
    ...
    y = load_logits_topk(X, stride_xm, n_expts_tot, offs_m, mask_m,
    ...
    for i in range(loop_iterations):
        ...
        y2 = tl.where(y_div[:, :, None] == offs_r_n[None, None, :],
                      (1 << y_rem)[:, :, None], 0)
