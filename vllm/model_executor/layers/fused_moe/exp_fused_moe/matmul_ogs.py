...
# utilities
from vllm.model_executor.layers.fused_moe.exp_fused_moe import meta
from vllm.model_executor.layers.fused_moe.exp_fused_moe.numerics import (
    InFlexData, OutFlexData, should_upcast_indices)
...
from .matmul_ogs_details._matmul_ogs import (_compute_writeback_idx,
                                             _finalize_split_k, _matmul_ogs,
                                             _matmul_postprocess)
from .matmul_ogs_details._p_matmul_ogs import (
    _p_matmul_ogs, get_per_device_per_stream_alloc_fn)
...
def matmul_ogs_torch(x, w, bias,
    ...
    for i, (lo, hi) in enumerate(offs):
        ...
        out = torch.matmul(round_x(x[batch, idx, :]).float(),
                           w[i, :, :].float())
        if bias is not None:
