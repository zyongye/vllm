...
def make_matmul_repr(base_name, order):
    ...
    def matmul_repr(specialization):
        ...
        dtypes = "x".join([
            convert_dtype(f"{signature[i][1:]}")
            for i in reorder(["Y", "X", "W"])
        ])
        layouts = "".join([
            f"{layout(i)}"
        ...
        blocks = "x".join([
            f"{constants[i]}"
            for i in ["BLOCK_M", "BLOCK_N", "BLOCK_K", "SPLIT_K"]
        ])
        mode = []
...
def matmul_launch_metadata(grid, kernel, args):
    ...
    ret["bytes"] = ((1 - skipped) * Y.numel() * Y.element_size() +
                    X.numel() * X.element_size() + n_w_bytes)
    return ret
