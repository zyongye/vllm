...
@triton.jit
def _compute_metadata_1(Hist, n_expts_tot, MDHist, MDTokStarts, MDTileStarts,
                        MDTileInfo, N_EXPTS_PAD: tl.constexpr,
...
@triton.jit
def _compute_metadata_2(Hist, n_expts_tot, MDHist, MDTokStarts, MDTileStarts,
                        MDTileInfo, N_EXPTS_PAD: tl.constexpr,
                        BLOCK: tl.constexpr, TILE_DIM: tl.constexpr):
...
def compute_metadata(routing_data, n_rows, block_m):
    ...
    _memset_metadata[(cdiv(metadata_size, MEMSET_BLOCK), )](
        metadata,
        metadata_size,  # inputs
        BLOCK=MEMSET_BLOCK  # optimization parameters
    )
    for kernel, num_blocks in [(_compute_metadata_1, 1),
                               (_compute_metadata_2, n_expts_tot)]:
        kernel[(num_blocks, )](
