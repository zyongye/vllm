...
from vllm.model_executor.layers.fused_moe.exp_fused_moe.meta import (
    cuda_capability_geq, rcp_max_finite)
...
def should_upcast_indices(*args):
    return any(tensor is not None and can_overflow_int32(tensor)
               for tensor in args)
...
def compute_scale(x, Out):
    ...
    return tl.fma(x_absmax, RCP_MAX_VALUE.to(tl.float32, bitcast=True),
                  1.0e-30)
