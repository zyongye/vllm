...
@torch.inference_mode()
def test_benchmark_fused_vs_unfused():
    """Benchmark fused Triton kernel vs unfused production path
    (CUDA RoPE + per_token_group_quant_fp8) across batch sizes 1..16384.

    This test is informational — it does not assert on timing.
    """
    num_heads = 64
    n_groups = 8
    heads_per_group = num_heads // n_groups
    ...
    print("\n")
    print(
        f"  {'Batch':>7s}  {'Unfused (ms)':>13s}  {'Fused (ms)':>12s}  {'Speedup':>8s}"
    )
    print(
        f"  {'-----':>7s}  {'------------':>13s}  {'-----------':>12s}  {'-------':>8s}"
    )
    ...
    for T in batch_sizes:
        ...
        positions = torch.randint(0, max_pos, (T,), device=device, dtype=torch.long)

        # Warmup unfused
        for _ in range(warmup):
            _unfused_inv_rope_fp8_quant(
                o.clone(),
                positions,
                cos_sin_cache,
                n_groups,
        ...
        end_event.record()
        torch.accelerator.synchronize()
        unfused_ms = start_event.elapsed_time(end_event) / iters
        ...
        print(f"  {T:>7d}  {unfused_ms:>13.3f}  {fused_ms:>12.3f}  {speedup:>7.2f}x")
