...
@pytest.fixture
def mlp_model_name():
    return "ibm-ai-platform/llama3-8b-accelerator"
...
def test_mlp_correctness(monkeypatch: pytest.MonkeyPatch,
                         test_prompts: list[list[dict[str, Any]]],
                         sampling_config: SamplingParams, model_name: str,
                         mlp_model_name: str):
    '''
    Compare the outputs of a original LLM and a speculative LLM
    should be the same when using mlp speculative decoding.
    '''
    with monkeypatch.context() as m:
        m.setenv("VLLM_USE_V1", "1")
